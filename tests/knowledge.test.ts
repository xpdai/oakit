...
describe('site content knowledge', () => {
  ...
  it('那莫好聽只保留目前提供的課程與新價格', () => {
    ...
    expect(knowledge).toContain('試上一堂課');
    expect(knowledge).toContain('限時體驗價 100元 （原價250元）');
    expect(knowledge).toContain('30 min 快速診斷+實作體驗');
    expect(knowledge).not.toContain('免費試上');
    ...
    expect(knowledge).toContain('試上一堂課｜價格 限時體驗價 100元 （原價250元）｜約 30 min 快速診斷+實作體驗');
    ...
    expect(tenant.services.find((service) => service.name === '試上一堂課')?.duration).toBe('30 min 快速診斷+實作體驗');
